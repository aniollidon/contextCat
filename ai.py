#!/usr/bin/env python3
"""
Script per generar llistes de paraules relacionades utilitzant l'API de Gemini.
"""
...
def get_api_key():
    """Obté la clau API de Gemini des de les variables d'entorn."""
    api_key = os.getenv("KEY_API_IA") or os.getenv("GEMINI_API_KEY") or os.getenv("API_KEY")
    if not api_key:
        print("Error: No s'ha trobat la clau API. Assegura't que KEY_API_IA, GEMINI_API_KEY o API_KEY estigui definida al fitxer .env")
        sys.exit(1)
    return api_key
...
def generate_words_for_concept(concept, api_key):
    """Genera una llista de paraules relacionades amb un concepte utilitzant l'API de Gemini."""
    prompt = f"Genera una llista de 100 noms i verbs únics en català relacionades amb el concepte de '{concept}'. Totes les paraules han d'estar en la seva forma singular. No s'accepten paraules compostes. Assegura que la paraula no contingui faltes d'ortografia i sigui correcte en català. El resultat ha de ser exclusivament un objecte JSON que contingui una única clau anomenada 'paraules' amb un array de les 100 paraules en format string. No incloguis cap text, explicació o format addicional fora de l'objecte JSON."
    ...
    try:
        ...
        if 'candidates' not in data or not data['candidates']:
            print(f"Error: Resposta inesperada de l'API per '{concept}'")
            return None
            
        text_response = data['candidates'][0]['content']['parts'][0]['text']
        
        # Neteja el text (elimina markdown si n'hi ha)
        ...
        try:
            ...
            if 'paraules' in result:
                return result['paraules']
            else:
                print(f"Error: Format JSON incorrecte per '{concept}' - no conté la clau 'paraules'")
                return None
        except json.JSONDecodeError as e:
            print(f"Error: No s'ha pogut parsejar el JSON per '{concept}': {e}")
            print(f"Resposta rebuda: {cleaned_text[:200]}...")
            return None
            
    except requests.exceptions.RequestException as e:
        print(f"Error en la petició HTTP per '{concept}': {e}")
        return None
...
def process_word(word, api_key, output_dir):
    """Processa una sola paraula."""
    print(f"\nGenerant paraules per: {word}")
    words_list = generate_words_for_concept(word, api_key)
    
    if not words_list:
        return False
    
    print(f"Rebudes {len(words_list)} paraules de Gemini")
...
def process_folder(folder_path, api_key):
    ...
    for json_file in json_files:
        ...
        if process_word(word, api_key, ai_folder):
            success_count += 1
        else:
...
def main():
    parser = argparse.ArgumentParser(description="Genera llistes de paraules relacionades utilitzant l'API de Gemini")
    ...
    if args.word:
        ...
        if process_word(args.word, api_key, output_dir):
        ...
        if process_folder(args.folder, api_key):
